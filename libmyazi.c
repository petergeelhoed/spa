...
struct azizen calcazi(double epoch, double lng, double lat)
{
    ...
    double geomMeanLong =
        fmod((280.46646 + julcent * (36000.76983 + julcent * 0.0003032)), 360) /
        RADPI;
    double geomMeanAnom =
    ...
    double meanOblElip =
        (23 + (26 + ((21.448 -
                      julcent * (46.815 +
                                 julcent * (0.00059 - julcent * 0.001813)))) /
                        60) /
                  60) /
        RADPI;
    double OblCorr =
    ...
    double eqOfTime =
        (vary * sin(2 * geomMeanLong) - 2 * eccentEarth * sin(geomMeanAnom) +
    ...
    double hourangle =
        ((trueSolTime < 0) ? trueSolTime / 4 + 180 : trueSolTime / 4 - 180) /
        RADPI;
    double SolZenith = acos(sin(lat / RADPI) * sin(sunDecl) +
